...
class BigDFTRelaxWorkChain(WorkChain):
    ...
    def define(cls, spec):
        super().define(spec)
        spec.expose_inputs(
            BigDFTBaseWorkChain, exclude=["parameters", "extra_retrieved_files"]
        )
        spec.input(
            "parameters",
        ...
        spec.input(
            "relax.threshold_forces",
            valid_type=orm.Float,
            required=False,
            default=lambda: orm.Float(0.0),
            help="energy cutoff value, in ev/Ang",
        )
        spec.input(
            "relax.steps",
            valid_type=orm.Int,
        ...
        spec.exit_code(101, "ERROR_FAILED_RELAX", "Subprocess failed for relaxation")

    def relax(self):

        self.ctx.inputs = AttributeDict(self.exposed_inputs(BigDFTBaseWorkChain))
        inputdict = self.inputs.parameters.dict
        if self.inputs.relax.perform:
            InputActions.optimize_geometry(
                inputdict, self.inputs.relax.algo.value, self.inputs.relax.steps.value
            )
        if self.inputs.relax.threshold_forces is not None:
            InputActions.dict_set(
                inputdict,
                "geopt",
                "forcemax",
        ...
        if self.inputs.extra_retrieved_files is not None:
            extra_retrieved_files.set_list(self.inputs.extra_retrieved_files.get_list())
        extension = "xyz"
    ...
    def results(self):
        ...
        if extension == "xyz":
            ...
            self.out(
                "total_energy", orm.Float(float(firstline[2]) * HARTREE_TO_EV).store()
            )
            ...
            try:
                ...
                s.set_cell(
                    ((line[1], 0.0, 0.0), (0.0, line[2], 0.0), (0.0, 0.0, line[3]))
                )
            except ValueError:
                try:
                    s._adjust_default_cell(
                        vacuum_addition=0.0, pbc=self.inputs.structure.pbc
                    )
                except ValueError:
            ...
            for i in log["positions"]:
                s.append_atom(symbols=list(i.keys())[0], position=list(i.values())[0])
        else:
            self.report(
